...
class Config:
    exclude_dirs = {'.git', '__pycache__', 'node_modules', '.vscode', '.idea', 'venv', 'env', '.venv'}
    max_size = 600000
    extensions = {'.py', '.js', '.jsx', '.ts', '.tsx', '.html', '.css', '.json', '.xml', '.yaml', '.yml', '.md', '.txt'}
...
def get_code_files_with_content(startpath, extensions=None, max_file_size=config.max_size):
    """
...
@click.option('--framework', '-f', help='Specify the framework (e.g., Fastapi, React, Django)')
@click.option('--max-size', '-m', default=config.max_size, help='Maximum file size to read (bytes)')
@click.option('--output', '-o', type=click.Choice(['structure', 'files', 'both']), default='both', help='Output format')
@click.option('--include-large', '-l', is_flag=True, help='Include large files (content will be skipped)')
def analyze_project(path, framework, max_size, output, include_large):
    ...
    if output in ['files', 'both']:
        ...
        for file_info in code_files:
            if not include_large and file_info['error'] and "too large" in file_info['error']:
                continue
